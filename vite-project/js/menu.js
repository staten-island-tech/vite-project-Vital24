const beef = [
  "Signature Marbled Beef",
  "Marbled Steak Cube",
  "Black Angus Denver Steak",
];
const pork = [
  "BBQ Susuage",
  "Spicy Pork Chop",
  "Pork Belly",
  "Apple Wood Bacon",
  "Spicy Pork Feet",
  "Kurobuta Pork Cheeks",
];
const All = [beef, pork];
console.log(All);

const data = [
  {
    name: "Chimasal",
    type: "meat",
    img: "https://www.cleaneatingmag.com/wp-content/uploads/2016/08/Beer-Marinated-Grilled-Flank-Steak-FEATURE.jpg?crop=1:1&width=1000",
  },

  {
    name: "Thin sliced brisket",
    type: "meat",
    img: "https://sizzlekoreanbbq.com/wp-content/uploads/2021/04/0007_Prime-Thinly-Sliced-Brisket.jpg",
  },

  {
    name: "Deungsim",
    type: "meat",
    img: "https://axwwgrkdco.cloudimg.io/v7/__gmpics__/98418da457834926b72c92c17247bd12?width=1000",
  },

  {
    name: "Kalbi",
    type: "meat",
    img: "https://static.onecms.io/wp-content/uploads/sites/44/2017/04/12/4502834.jpg",
  },
  {
    name: "Short Ribs",
    type: "meat",
    img: "https://www.theflavorbender.com/wp-content/uploads/2018/04/Galbi-Korean-BBQ-Short-Ribs-Feat2.jpg",
  },
  {
    name: "Gimbap Roll",
    type: "sushi",
    img: "https://www.cleaneatingmag.com/wp-content/uploads/2016/08/Beer-Marinated-Grilled-Flank-Steak-FEATURE.jpg?crop=1:1&width=1000",
  },

  {
    name: "California Roll",
    type: "sushi",
    img: "https://m.media-amazon.com/images/I/61GGkLruQuL.jpg",
  },

  {
    name: "Shrimp Tempura Roll",
    type: "sushi",
    img: "https://norecipes.com/wp-content/uploads/2022/02/shrimp-tempura-roll-004.jpg",
  },

  {
    name: "Rainbow Roll",
    type: "sushi",
    img: "https://foreignfork.com/wp-content/uploads/2022/04/RainbowRollFEATURE.jpg",
  },
  {
    name: "Dragon Roll",
    type: "sushi",
    img: "https://properfoodie.com/wp-content/uploads/2020/07/featured-dragon-roll-sushi-9.jpg",
  },
  {
    name: "Gimbap Roll",
    type: "sushi",
    img: "https://www.cleaneatingmag.com/wp-content/uploads/2016/08/Beer-Marinated-Grilled-Flank-Steak-FEATURE.jpg?crop=1:1&width=1000",
  },

  {
    name: "California Roll",
    type: "sushi",
    img: "https://m.media-amazon.com/images/I/61GGkLruQuL.jpg",
  },

  {
    name: "Shrimp Tempura Roll",
    type: "sushi",
    img: "https://norecipes.com/wp-content/uploads/2022/02/shrimp-tempura-roll-004.jpg",
  },

  {
    name: "Rainbow Roll",
    type: "sushi",
    img: "https://foreignfork.com/wp-content/uploads/2022/04/RainbowRollFEATURE.jpg",
  },
  {
    name: "Dragon Roll",
    type: "sushi",
    img: "https://properfoodie.com/wp-content/uploads/2020/07/featured-dragon-roll-sushi-9.jpg",
  },
];

export { beef };
export { data };
